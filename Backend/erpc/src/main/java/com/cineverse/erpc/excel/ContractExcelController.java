...
public class ContractExcelController {
    ...
    public void contractDownload(@PathVariable long contractId, HttpServletResponse response) throws IOException {
        ...
        String[] headers = {
                "계약서 코드", "거래처명", "거래코드", "납부형식", "총비용", "계약금", "중도금", "잔금",
        ...
        row.createCell(3).setCellValue(contract.getContractCategory().getContractCategory());
        row.createCell(4).setCellValue(contract.getContractTotalPrice());

        Cell downPaymentCell = row.createCell(5);
        ...
        Cell progressPaymentCell = row.createCell(6);
        ...
        for (ContractProductDTO product : contract.getContractProduct()) {
            ...
            productRow.createCell(8).setCellValue(product.getProduct().getProductName());
